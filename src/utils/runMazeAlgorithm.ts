...
export const runMazeAlgorithm = async({maze,grid,startTile,endTile,setIsDisabled,speed}:{
    maze:MazeType;
    grid:GridType;
    startTile:TileType;
    endTile:TileType;
    ...
    if(maze=="BINARY_TREE"){
        ...
        const currentSpeed = SPEEDS.find(e=>e.value===speed)!.value??2;
        //construct boarder
        await constructBorder(grid,startTile,endTile);
        await recursiveDivision({grid,startTile,endTile,row:1,col:1,height:Math.floor((MAX_ROWS-1)/2),width:Math.floor((MAX_COLS-1)/2),setIsDisabled,speed});
        setTimeout(()=>{
            setIsDisabled(false);
        },800*currentSpeed);
    }
