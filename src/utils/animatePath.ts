...
export const animatePath  =(traversedTiles:TileType[],path:TileType[],startTile:TileType,endTile:TileType,speed:SpeedType)=>{
    ...
    for(let i = 0;i<traversedTiles.length;i++){
        ...
        },SLEEP_TIME*i*SPEEDS.find((e)=>e.value===speed)!.value);
    }
    ...
    setTimeout(()=>{
        ...
        for(let i=0;i<path.length;i++){
            ...
            },EXTENDED_SLEEP_TIME*i*SPEEDS.find((e)=>e.value===speed)!.value);
        }

    },SLEEP_TIME*traversedTiles.length*SPEEDS.find((e)=>e.value===speed)!.value);
}